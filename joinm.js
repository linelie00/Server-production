...
app.post('/login_post',(req, res) => {
    ...
    const sql = 'SELECT * FROM joinmb WHERE id = ?';
...
app.post('/join_post', (req, res) => {
    ...
    if(user_name === '' || id === '' || pw === '' ) {
        ...
        // 중복된 아이디 있는지 확인
        const dupCheckQuery = 'SELECT * FROM joinmb WHERE id = ?';
        connection.query(dupCheckQuery, [id], (checkErr, checkResults) => {
            ...
            if (checkResults.length === 0) {
                ...
                const sql = 'INSERT INTO joinmb (user_name, id, pw, old_pw, join_date, update_date) VALUES (?, ?, ?, ?, ?, ?)';
                const values = [user_name, id, pw, pw, currentDateTime, currentDateTime];
  ...
  app.post('/update_post', (req, res) => {
    ...
    const checkQuery = 'SELECT * FROM joinmb WHERE id = ? AND pw = ?';
    connection.query(checkQuery, [old_id, old_pw], (checkErr, checkResults) => {
      ...
      if (checkResults.length === 0) {
        ...
        const updateQuery = 'UPDATE joinmb SET user_name = ?, id = ?, pw = ?, old_pw = ?, update_date = ? WHERE id = ? AND pw = ?';
  ...
  app.post('/delete', (req,res) => {
    ...
    if (checkResults.length === 0) {
        ...
        const deleteQuery = 'DELETE FROM joinmb WHERE id = ? AND pw = ?';
...
app.post('/updateMultiple', (req, res) => {
    ...
    const checkSql = `SELECT * FROM joinmb WHERE ${where};`;
    connection.query(checkSql, (checkErr, checkResults) => {
        ...
        if (checkResults.length === 0) {
            ...
            const sql = `UPDATE joinmb SET ${set}, update_date = ? WHERE ${where};`;
